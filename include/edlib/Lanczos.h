  ...
  namespace gf {
    ...
    protected:
      ...
      std::complex<double> freq_point(int index) {
        return std::is_base_of<alps::gf::real_frequency_mesh, Mesh>::value ? std::complex<double>(_omega.points()[index], p["lanc.IDELTA"]) : std::complex<double>(0.0, _omega.points()[index]);
      };
